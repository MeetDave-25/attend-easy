...
interface ModernSplashScreenProps { onComplete: () => void; brandName?: string; }
const notes = [164.81, 196, 246.94, 293.66, 329.63, 246.94];
...
function playNote(context: AudioContext, frequency: number) {
  const oscillator = context.createOscillator();
  const gain = context.createGain();
  oscillator.type = "sine";
  oscillator.frequency.setValueAtTime(frequency, context.currentTime);
  gain.gain.setValueAtTime(0.0001, context.currentTime);
  gain.gain.exponentialRampToValueAtTime(0.028, context.currentTime + 0.025);
...
const ModernSplashScreen = ({ onComplete, brandName = "AttendEasy" }: ModernSplashScreenProps) => {
  ...
  const startSound = () => {
    try {
      ...
      setSoundOn(true);
      let index = 0;
      const tick = () => { if (audioRef.current) playNote(audioRef.current, notes[index++ % notes.length]); };
      tick();
      if (!loopRef.current) loopRef.current = window.setInterval(tick, 520);
    } catch { setSoundOn(false); }
  };

  const toggleSound = () => {
    if (soundOn) { if (loopRef.current) window.clearInterval(loopRef.current); loopRef.current = null; setSoundOn(false); return; }
    startSound();
  };
  ...
  useEffect(() => {
    ...
    timeline.fromTo(".launch-kicker, .launch-title, .launch-subtitle", { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: 0.7, stagger: 0.08, ease: "power3.out" }, "-=0.1");
    ...
    return () => { timeline.kill(); if (loopRef.current) window.clearInterval(loopRef.current); if (audioRef.current) void audioRef.current.close(); };
  }, [onComplete]);

  return (
    <div ref={screenRef} className="launch-screen">
      ...
      <div className="launch-content">
        ...
        <h1 className="launch-title">{brandName}</h1><p className="launch-subtitle">A clearer rhythm for every campus.</p><div className="launch-progress"><div ref={progressRef} /></div><div className="launch-meta"><span>BUILD 03.01</span><span>PREPARING WORKSPACE</span><span>READY</span></div>
      </div>
    </div>
  );
};
