...
const ModernSplashScreen = ({ onComplete }: ModernSplashScreenProps) => {
  ...
  useEffect(() => {
    ...
    letterRefs.current.forEach((letter, index) => {
      if (!letter) return;
      timeline.to(letter, { opacity: 1, rotationY: 0, y: 0, duration: 0.55, ease: "back.out(1.5)", onStart: () => { if (audioRef.current && soundOnRef.current) playFlipSound(audioRef.current, index); } }, index === 0 ? "-=0.6" : "+=0.12");
    });
  ...
  return (
    <div ref={screenRef} className="launch-screen">
      ...
      <div className="launch-content">
        ...
        <h1 className="launch-title" aria-label="LJCCA">{"LJCCA".split("").map((letter, index) => <span key={`${letter}-${index}`} ref={(element) => { letterRefs.current[index] = element; }} className={index < 2 ? "launch-letter-red" : "launch-letter-white"}>{letter}</span>)}</h1><p className="launch-subtitle">A clearer rhythm for every campus.</p><div className="launch-progress"><div ref={progressRef} /></div><div className="launch-meta"><span>BUILD 03.01</span><span>PREPARING WORKSPACE</span><span>READY</span></div>
      </div>
    </div>
  );
};
